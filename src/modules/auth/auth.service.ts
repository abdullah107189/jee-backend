...
export const authService = {
  async register(input: RegisterInput): Promise<AuthResponse> {
    ...
    if (existingByPhone) throw new AppError("An account with this phone already exists", 409);
  ...
  async login(input: LoginInput): Promise<AuthResponse> {
    ...
    if (!verifyPassword(input.password, user.password)) throw new AppError("Invalid credentials", 401);
  ...
  async forgotPassword(input: ForgotPasswordInput): Promise<ForgotPasswordResponse> {
    ...
    const resetToken = signToken(
      { sub: user.id, purpose: AUTH.TOKEN_PURPOSE.RESET_PASSWORD },
      config.jwtSecret,
      AUTH.RESET_TOKEN_EXPIRY_SECONDS,
    );
