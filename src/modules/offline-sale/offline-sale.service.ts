...
import type {
  ...
  ListOfflineSalesResult,
  OfflineSaleQuery,
  OfflineSaleWithRelations,
} from "./offline-sale.type";
...
export const offlineSaleService = {
  async list(query: OfflineSaleQuery, viewer?: { role: UserRole; userId: string }): Promise<ListOfflineSalesResult> {
  ...
  async getById(id: string): Promise<OfflineSaleWithRelations> {
    const sale = await offlineSaleRepository.findById(id);
    if (!sale) throw new AppError(OFFLINE_SALE_MESSAGES.NOT_FOUND, 404);
    return sale;
  },

  async create(sellerUserId: string, input: CreateOfflineSaleInput): Promise<OfflineSaleWithRelations> {
    ...
    if (!item) throw new AppError(OFFLINE_SALE_MESSAGES.ITEM_UNAVAILABLE, 400);
    if (item.status !== "AVAILABLE") throw new AppError(OFFLINE_SALE_MESSAGES.ITEM_UNAVAILABLE, 400);
    ...
    const data: Prisma.OfflineSaleUncheckedCreateInput = {
  ...
  async update(id: string, input: Prisma.OfflineSaleUpdateInput): Promise<OfflineSaleWithRelations> {
